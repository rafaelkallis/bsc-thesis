...
for dataset, data in [("synthetic", data_synthetic), ("real", data_real)]:
    ...
    plot(data[15000]["ticks"]/100, [median(w) for w in sliding_window(data[15000]["query_runtime"],10)], label="15s")
    plot(data[30000]["ticks"]/100, [median(w) for w in sliding_window(data[30000]["query_runtime"],10)], label="30s")
    legend()
    ylim(ymax=300)
    ...
    periods = sorted(list(data.keys()))

    xlabel("GC Periodicity")
    ylabel("Avg. Query Runtime [ms]")
    plot(taus, [median(data[p]["query_runtime"][995:1005]) for p in periods], linestyle="-", marker="o")
    ylim(ymax=190)
