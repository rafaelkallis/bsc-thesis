...
The experimental methodology used on GC is also applied on QTP in \Cref{sec:qtp-experiment}.
...
As time passes, the index becomes more static (see \Cref{sec:query-runtime}), 
...
unproductive nodes. Thit is the case because content nodes have to be drawn twice in order
...
An increase in time yields an increase in traversed unproductive nodes, an observation
depicted in \Cref{fig:upt_unprod_nodes_synthetic,fig:upt_unprod_nodes_real}, as well.
...
Thit is the case because we use a running median \cite{running-median}.
...
runtime over the query period, i.e.\ the average time in between two consecutive
...
On the other hand, as queries become more infrequent, periodic GC is preferred.
...
we execute a query every $1.1$ seconds, we are indifferent choosing between GC
...
%of a query under QTP increases QTP's cost superlinearly, and its write cost
%in particular. If this is the case, increasing the production rate harms
%queries under QTP more compared to GC. 
We conjecture that the cost of a query under QTP increases at a higher rate
compared to GC if the number of traversed unproductive nodes increases.
This would also explain why the slope on QTP is steeper compared to GC, 
with respect to the query period 
(cf.\ \Cref{fig:gc_vs_qtp_synthetic,fig:gc_vs_qtp_real}).
If this is the case, the rate at which unproductive nodes are produced
impacts the horizontal offset (x-coordinate) of the equilibrium point.
If we produce more unproductive nodes during a fixed
interval, the equilibrium point might move towards the left. 
Under such circumstances, if queries are faster under QTP and we increase the unproductive node production rate,
the queries might then suddenly run faster on GC.
%and therefore make GC more beneficial for 
...
\begin{itemize}
    ...
    \item Queries under QTP traverse near no unproductive nodes throughout
        ...
        because of unproductive nodes (\Cref{sec:qtp-experiment}).
...
that deciding between one of the two approaches depends, amongst others, on how often queries occur.
