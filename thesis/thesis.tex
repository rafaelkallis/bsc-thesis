...
The experiments last for 30 seconds and since the sliding window has length $L = 30s$,
there are no unproductive nodes during the experiments.
We observe that queries with QTP tend to require double the time queries need
...
three update to query ratios: 1, 10 and 10 updates per query. An increase
...
The latter Figures show the number of traversed unproctive nodes over update to
query ratio.
A node becomes volatile if it updated $t$ during a sliding window of length $L$.
...
are of running both periodic GC and QTP on the database system.
