...
Depending on the data model, applications might use a hierarchical \cite{Hainaut2009-hierarchical} database
...
When a user submits a new post, a job \cite{Job} is sent to the CMS for processing. % If the
...
compare GC and QTP under different parameterizations and workloads.
...
index nodes are retained and less nodes are pruned. As a result, fewer nodes
...
spread (almost) uniformly amongst nodes. As a result, less nodes become volatile,
...
t.u.\ interval. Executing less update operations during a fixed time interval
implies that less nodes become volatile, and later on, unproductive.
...
to
\Cref{fig:period_runtime_synthetic,fig:period_runtime_real}
it becomes more clear that although we have less unproductive
...
runtime. The main overhead added by QTP is checking if the visited 
...
Writing changes might also become a large overhead when there
are many unproductive nodes.
...
\begin{figure}[h]
  ...
  \begin{subfigure}{0.49\linewidth}
    \centering
    \caption{Synthetic}
    \includegraphics[width=8cm]{gc_vs_qtp_synthetic}
    \label{fig:gc_vs_qtp_synthetic}
  \end{subfigure}
  \begin{subfigure}{0.49\linewidth}
    \centering
    \caption{Real-World}
    \includegraphics[width=8cm]{gc_vs_qtp_real}
...
and QTP. The same holds in \Cref{fig:gc_vs_qtp_real} for $25$ updates
...
update to query ratio (\Cref{sec:update-query-ratio}), amongst others.
...
If we produce less unproductive nodes, the equilibrium point might move towards the right
...
Our main findings w.r.t.\ the production rate of unproductive nodes are:
...
\begin{itemize}
    ...
    \item The production rate of unproductive nodes is the highest with skew
        ...
        less unproductive nodes (\Cref{sec:skew}).
    ...
    \item Queries under QTP traverse near no unproductive nodes throughout
        ...
        from 11.5ms to 20ms. The main overhead of QTP is checking if visited
        nodes have children, which is done to determine unproductive nodes
        (\Cref{sec:qtp-experiment}).
...
Furthemore, we did not investigate what the benefits and caveats
