...
\begin{algorithm}
  ...
  \KwData{Index node $n$}
  % \vspace{2.6mm}
  \For{node $d \in desc(n)$ in bottom-up fashion}{
 ...
 */
void cleanIndexWAPI(NodeState n) {
    for (Node node : filter(
            (Node n) -> n.getState().getChildNodeCount(1) == 0 &&
                        !n.getState().getBoolean("match") &&
                        !n.getState().isVolatile()
            ),
            bottomUp(n)
    ) {
        node.getState().builder().remove();
    }
}
\end{verbatim}
\end{figure}
...
Iterable<String> QueryQTP(String k, String v, String m, NodeStore store) {
    ...
    String indexRootPath = join(OAK_INDEX_PREFIX, k, OAK_INDEX_INTERNAL, v); 

    /* e.g: /oak:index/pub/:index/now/a */
    String targetNodePath = join(indexRootPath, m);

    NodeState targetNode = getNode(targetNodePath, store);

    /* map matching index nodes' path to corresponding content nodes' path */
    return map(
        (Node node)-> truncate(indexRootPath, node.getPath()),

        /* filter non-matching index nodes */
        filter(
            (Node node) -> {
                boolean matching = node.getState().getBoolean("match");

                /* prune if no children, is not matching and is not volatile */
                if (node.getState().getChildNodeCount(1) == 0 &&
                    !matching &&
                    !node.getState().isVolatile()
                ) {
                    node.getState().builder().remove();
                }
                return matching;
            ...
            bottomUp(targetNode, targetNodePath)
        );
    );
}
\end{verbatim}
    ...
    %     targetNode,
    %     targetNodePath,
    %     (NodeState n, String path) -> {
    %         boolean matching = n.getBoolean("matching");
    %         if (n.getChildNodeCount(1) == 0 &&
    %             !matching &&
    %             !n.isVolatile()
    %         ) {
    %             n.builder().remove();
    %         } else if (matching) {
    %             result.add(truncate(indexRootPath, path));
    %         }
    %     }
    % );
    % return result;
...
\begin{verbatim}
/**
 * Node class containing information about a node's state,
 * path and depth. 
 */
class Node {
    ...
    Integer getDepth() {
        if (depth == null) {
            depth = path.length() - path.replace("/","").length();
        }
        return depth;
    }
}
\end{verbatim}
  \caption{\texttt{Node} class implementation}
  \label{fig:java_node}
\end{figure}
...
% \begin{verbatim}
% /**
%  * Applies given function to all descendants of node n in post-order fashion
%  *
%  * @param n: Node to apply traversal on
%  * @param path: Path of node
%  * @param func: function to be applied on nodes
%  */
% void postOrder(NodeState n, String path, BiConsumer<NodeState, String> func) {
%     for (String lambda : n.getChildNodeNames()) {
%         NodeState child = n.getChildNode(lambda);
%         postOrder(child, join(path, lambda), func);
%     }
%     func.accept(n, path);
% }
% \end{verbatim}
%   \caption{\texttt{postOrder()} implementation}
%   \label{fig:java_post_order}
% \end{figure}
...
\begin{verbatim}
/**
 * Returns an iterable which lazily traverses a subtree rooted at n
 * in a bottomUp fashion.
 *
 * @param n: Node to apply traversal on
 * @returns: iterable for tree traversal
 */
Iterable<Node> bottomUp(Node n) {
    return () -> {
        ...
        return new Iterator<Node>() {
            ...
            public Node next() {
                while (s1.size() > 0 && (s2.size() == 0 || 
                          s1.peek().getDepth() > s2.peek().getDepth())
                ) {
                    Node n = s1.pop();
                    s2.push(n);
...
Iterable<Node> bottomUp(NodeState n) { return bottomUp(new Node(n, "/")); }
\end{verbatim}
  \caption{\texttt{bottomUp()} implementation}
  \label{fig:java_bottom_up}
\end{figure}
...
NodeState getNode(String path, NodeStore store) {
    ...
    for (String lambda : split(path)) {
        n = n.getChildNode(lambda);
    }
}
\end{verbatim}
  \caption{\texttt{getNode()} implementation}
  \label{fig:java_get_node}
\end{figure}
...
\end{verbatim}
  \caption{\texttt{split()} implementation}
  \label{fig:java_split}
\end{figure}

\begin{figure}
  \centering
\begin{verbatim}
 * Truncates a given prefix from a string
 */
String truncate(String prefix, String original) {
    if (!original.startsWith(prefix)) { throw new RuntimeError(); }
    return original.substring(prefix.length);
} 
\end{verbatim}
  \caption{\texttt{truncate()} implementation}
  \label{fig:java_truncate}
\end{figure}
