...
distribution picks a small subset of nodes more frequently than others. 
...
The number of traversed unproductive nodes increases sublinearly as update
operations increase. We suggest that the number of traversed unproductive nodes
has an upper bound. Obviously, the index subtree can not have
more nodes than the content subtree, because the index subtree is a subset
of the content subtree.
Thus, the number of index nodes and therefore
also the number of unproductive index nodes, have an upper bound. This explains
why we see the number of traversed unproductive nodes converge sublineary.
They converge towards this upper bound.
...
The number of unproductive nodes increases sublinearly. We explained earlier that
there is an upper bound to the number of unproductive index nodes, hence 
...
to the sliding window length $L$. We observe greater sliding window lengths 
to cause a sublinear increase to the number of unproductive nodes traversed 
by WAPI during query execution. Since there
is a maximum to the amount of unproductive index nodes as explained earlier, 
the function must converge. The Figures seem to suggest a linear relationship
...
produce during the entire experiment. We expect to see the function converge
if we run the experiment for a significantly longer duration of time.
...
less nodes get frequently updated and therefore the total number of nodes
...
steep decrease in unproductive nodes when skew is $s=1.5$. 
The hotspot reaches a point where it loses a significant amount of nodes because it is
so skewed. From there on we see a sublinear rate of convergence. 
%When $s \to \infty$,
...
We conjecture that the high density of nodes in the synthetic dataset is responsible for the
observed phenomenon.
...
Figures TEST and TEST show the average query runtime with respect to period $T$.
The optimal $T$ seems to be around $T=5s$. If $T<5s$, GC starts stealing CPU time
from the query executor and queries take longes to process.
On the other hand, when $T>5s$, the query runtime increases sublinearly because
the index fills up with unproductive nodes. We see a sublinear increase since
unproductive nodes have an upper bound set by the number of content nodes, as
...
Let us consider \Cref{fig:periodicity_unprod_nodes_synthetic,fig:periodicity_unprod_nodes_real}.
