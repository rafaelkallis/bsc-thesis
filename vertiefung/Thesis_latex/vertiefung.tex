...
The following pages describe how nodes are inserted, queried and deleted from the WAPI.
Next, we describe how volatility is computed.
Finally, a reference implementation in Java is presented.
...
\begin{figure}[h]
    \begin{center}
        \begin{tikzpicture}
            ...
            \node (mongo_desc) at (0, -3) {\footnotesize Persistence Tier};
...
The process ends if we propagate up to \texttt{/index} or reach a node that violates at least one of the above three conditions.
...
\end{algorithm}
\begin{scriptsize}
    Where $chd(n)$ is the set of children of node $n$.
\end{scriptsize}
...
Let $n^i$ denote version $i$ of node $n$.
$n^i$ is a member of the node set $N(G^i)$ of snapshot $G^i$.
...
A value is a boolean variable which is $\top$ iff $n$ was added at the point of time indicated by the revision key.
\begin{example}
...
Notice that the \texttt{keySet} referred to in the Java code is based on Java's ordered sets and is ordered by the revision timestamp.
...
Performance suffers if looking at split documents is required while computing the volatility count.
We modify Oak's document splitting implementation as mentioned in~\cite{KW17} in order to prevent split document lookups.
Essentially, all necessary information is kept in a document to: (a) compute the volatility count and (b) prevent race conditions.
...
\begin{example}
    ...
    ``$t(r)$'' is the point of time revision $r$ was committed. Only the day, hours and minutes are shown for brevity.
    ...
    Finally, any following revisions committed by the local Oak instance (i.e, $r_7, r_8, r_9$) are moved to the split document since there are already $\tau$ revisions in the document, enough to decide the node's volatility.
\end{example}
...
\Cref{fig:helper_functions} presents the Java implementation of two helper functions.
