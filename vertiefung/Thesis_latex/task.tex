...
\begin{algorithm}[H]
    ...
    \begin{footnotesize}
        \KwData{Triple $(k, v, m)$, where $k$ is a property, $v$ a value and $m$ a node.}
        \Begin{
            $n^j \longleftarrow \texttt{/index/k/v/m}$\;
            $n^j[k] \longleftarrow \textsc{nil}$\;
            \While{$n^j[\lambda] \neq \texttt{index} \land chd(n^j) = \emptyset \land n^j[k] \neq v \land \textcolor{blue}{\lnot \ isVolatile(n^j,H_i)}$}{
                $u^j \longleftarrow n^j$\;
                $n^j \longleftarrow par(n^j)$\;
                remove node $u^j$\;
            }
        }
    \end{footnotesize}
...
\begin{definition}
    ...
    Node $n^j$ is a member of the nodeset of snapshot $G^j$, i.e $n^j \in N(G^j)$.
